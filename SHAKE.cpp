...
# include <time.h> //для задержки
// библиотека, нужна для использования функции Sleep()
# include <conio.h>
// библиотека, нужна для использования функций kbhit()(при нажатии клавиши возвращает 1) и getch()(чтение символа с клавы)
using namespace std;
char main_color[] = "color ##";
// шаблон для цвета
int snake_size, change_x, change_y, coordinates_x[1000], coordinates_y[1000], food_x = -1, food_y = -1, score=0;
/*
...
*/
const int N = 13, M = 17, INTERVAL = 300;
/*
    константы,
...
*/
void change_color()
// функция изменения цвета консоли(пасхалочка)
{
...
// функция, считывающая нажатую клавишу
{
    symbol = getch();
     ...
     switch (symbol)
     {
         case 72: if(change_x != 1 || change_y != 0) { change_x = -1; change_y = 0; } break;//вверх
         case 75: if(change_x != 0 || change_y != 1) { change_x = 0; change_y = -1; } break;//влево
         case 80: if(change_x != -1 || change_y != 0) { change_x = 1; change_y = 0; } break;//вниз
         case 77: if(change_x != 0 || change_y != -1) { change_x = 0; change_y = 1; } break;//вправо(по индексам функц клавиш)
         // управление змейкой через клавиши вверх-вниз
         case 32 : change_color(); //код пробела = 32
		 break;
...
{
     if(change_x == 1 && change_y == 0) a[coordinates_x[1]][coordinates_y[1]] = 'v'; // нажата клавиша вниз
...
// функция добавления еды на карту
{
     ...
     for (int i = 1; i <= 9; ++i)
     {
         ...
         if(x == 0) ++x;
         if(y == 0) ++y;
         if(a[x][y] != 'o' && a[x][y] != 'v' && a[x][y] != '^' && a[x][y] != '<' && a[x][y] != '>')
         {
             food_x = x;
             food_y = y;
             a[x][y] = 'X';
